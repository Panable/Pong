...
public class Paddle {
	...
	public Paddle(int x, int y, int speed, Keyboard key, Controller controller) {
		m_x = x; m_y = y; m_speed = speed; m_key = key;  m_controller = controller;
	}
	
	public void update() {
		if (m_controller == Controller.one) {
			if (m_key.isW()) m_y -= m_speed;
			if (m_key.isS()) m_y += m_speed;
		} else {
			if (m_key.isUp()) m_y -= m_speed;
			if (m_key.isDown()) m_y += m_speed;
		}
		
		if (m_y < 0) m_y = 0;
		if (m_y >= Pong.HEIGHT - height) m_y = Pong.HEIGHT - height;
	}
